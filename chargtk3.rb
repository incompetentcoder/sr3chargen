...
#      @tooltips.set_tip(@windows,"Select a totem to be able to select spells",nil)
      @windows.tooltip_text="Select a totem to be able to select spells"
    else
#      @tooltips.set_tip(@windows,"",nil)
      @windows.tooltip_text=""
    end
...
class Attributeblock < Gtk::Frame
  ...
  def disabletotem
    @totem.each do |x|
      x.active = -1 if x.class == Gtk::ComboBoxText
      x.sensitive = false
    end
 #   @tooltips.set_tip(@totem[3],'',nil)
    @totem[3].tooltip_text=""
    5.times {|x| @totem[1].remove(0)}
    @totemsenabled = nil
  end
...
#    @tooltips.set_tip(@header[:ACT],"Actual effective Attribute after all Modifiers",nil)
    @header[:Attributes].tooltip_text=fetchtp(nil,1)
    @header[:Points].tooltip_text="Build Points, 2 to raise Attribute by 1"
    @header[:RM].tooltip_text="Racial Modifier to base Attribute"
    @header[:BA].tooltip_text="Base Attribute after applying racial Modifiers"
    @header[:CBM].tooltip_text="Cyberware/Bioware Modifiers to base Attribute"
    @header[:MM].tooltip_text="Magic/Power Modifiers to base Attribute"
    @header[:ACT].tooltip_text="Actual effective Attribute after all Modifiers"
    ...
    CONSTANT[:attributes].each_with_index do |x, y|
      ...
      @attributes[x][:Attributes].tooltip_text=fetchtp(x.to_s)
    end
    ...
    @special.each do |x|
      ...
      x[1][0].tooltip_text=fetchtp(x[0].to_s)
    end
    ...
    @table.attach @totem[1] = Gtk::ComboBoxText.new,3,10,24,26,*ATCH
    @table.attach @totem[2] = Gtk::Label.new('Totem'),0,3,26,28,*ATCH
    @table.attach @totem[3] = Gtk::ComboBoxText.new,3,10,26,28,*ATCH
...
#          (temp[:properties] ? temp[:properties] : ''),nil)
        @totem[3].tooltip_text=(temp[:desc] ? temp[:desc]+"\n" : '')+
...
#    @window.show_all
 ...
 #       @tooltips.set_tip(@header[:Skill][1],CONSTANT[:activeskills][getattr][x.active_text.to_sym][:Desc],nil)
        @header[:Skill][1].tooltip_text = CONSTANT[:activeskills][getattr][x.active_text.to_sym][:Desc]
      else
 #       @tooltips.set_tip(@header[:Skill][1],nil,nil)
        @header[:Skill][1].tooltip_text = ""
      end
    end
...
class Spellblock < Gtk::Frame
  ...
  def initialize(app)
    ...
    @vbox.pack_start(@win,true,true,0)
    @vbox.pack_start(@win2,true,true,0)
    @vbox.show_all
    add(@vbox)
  end
end
