...
CONSTANTS={
  ...
  :spelltypes => {
	...
	Manipulation: [:Control, :Elemental, :Transformation, :Telekinetic]
  },
  :nuyen => {
  -5 => 500, 0 => 5000, 5 => 20000, 10 => 90000, 15 => 200000,
  20 => 400000, 25 => 650000, 30 => 1000000
  },
  :grades => {
  ...
  :activeskills => {
	...
	:Intelligence => [
	  :'Armor Br', :'Assault Rifle Br', :'Aura Reading', :'Bike Br', :Biotech,
	  :'Biotech Br',:'Blowgun Br', :'Car Br',:'Clubs Br', :'Computer Br',
	  :Computers, :'Cyberimplant Br', :Demolitions, :'Demolitions Br', :Disguise,
	  :'Diving Br', :'Edged Br', :Electronics, :'Electronics Br', :Gunnery,
	  :'Gunnery Br', :'Gyrojet Br', :'Heavy Br', :'Hovercraft Br',
	  :'Interrogation Br', :'Laser Br', :'Launchers', :'Launchers Br', :'LTA Br',
	  :'Motorboat Br', :'Parachute Br', :'Pistols Br', :'Polearms Br', 
	  :'Projectile Br', :'Rifle Br', :'Rotor Br', :'Sailboat Br', 
	  :'Semiballistic Br', :'Ship Br', :'Shotgun Br', :'Small Unit Tactics',
	  :'Sprayer Br', :'Stealth Br', :'Suborbital Br', :'SMG Br', :'Throwing Br',
	  :'Unarmed Br', :'Vector Thrust Br', :'Walker Br', :'Whip Br', :'Winged Br',
	  ],
	:Quickness => [
	  :Ambidexterity, :'Assault Rifle', :Blowgun, :Gyrojet, :Laser, :'Oral Gun',
	  :Pistol, :Rifle, :Shotgun, :Stealth, :SMG, :Whip
	  ],
	:Reaction => [
	  :Bike, :Car, :Hovercraft, :LTA, :'Mechanical Arm', :Motorboat, :Rotor,
	  :Sailboat, :Semiballistic, :Ship, :Submarine, :Suborbital, :Tracked,
	  :'Vector Thrust', :Walker, :Winged
	  ],
	:Strength => [
	  :Aikido, :'Arnis de Mano', :Boxing, :Brawling, :Carromeleg, :Clubs, 
	  :Cyberimplant, :Edged, :Escrima, :Heavy, :Karate, :'Kung Fu',
	  :'Muay Thai', :Ninjitsu, :'Pentjak Silat', :Polearms, :Projectile,
	  :Sprayer, :'Tae Kwon Do', :'Tai Chi Chuan', :Throwing, :Underwater,
	  :Wildcat
	  ],
	:Willpower => [:Centering, :Conjuring, :Divining, :Enchanting, :Sorcery],
  },
  :totems => {
	:animal => {
	  :badger => {
	  ...
	  :dog => {
		...
		spirits: [[:Field,2],[:Hearth,2]], desc: 'Willpower(6) test to change
		course of action, complex action'.delete("\n\t"),
		properties: 'Loyal, Generous, Stubborn, Savage Defender of Home'},
	  :dolphin => {
		environment: 'On or by the sea', spells: [[:Combat,-1],[:Detection,2]],
		spirits: [[:Sea,2]],
		properties: 'Playful, Wise, Protectors, Helpers, Anti-Evil'},
	  :dove => {
		environment: 'Forests', spells: [[:Health,2],[:Detection,1]],
		spirits: [[:Sky,1]], desc: 'Cannot cast Combat spells, Willpower(6) Test 
		to purposefully inflict harm on a metahuman'.delete("\n\t"),
	  ...
	  :fish => {
		environment: 'On or near water', spells: [[:Detection,2],[:Combat,-1]],
    spirits: [[:Choose,[[:Water,1]]]], desc: 'Choose 1 spirit of water',
		properties: 'Clever, Quick, Insightful'},
	  :fox => {
		environment: 'Anywhere on land', spells: [[:Illusion,2],[:Combat,-1]],
    spirits: [[:Choose,[[:Land1,2],[:Man1,2]]]], desc: 'Willpower(6) Test to 
    spare fallen enemy, Choose one Spirit of Land or Man'.delete("\n\t"),
		properties: 'Sly, Clever, Thief, Trickster'},
	  :gator => {
		environment: 'Swamp,River,Urban',
		spells: [[:Combat,2],[:Detection,2],[:Illusion,-1]],
    spirits: [[:Choose,[[:Swamp,2],[:Lake,2],[:River,2],[:City]]]], desc: 
    'Willpower(6) Test to break off fight or  conflict, as wilderness 
    ...
    [:Manipulation,2]]],[:Combat,-1]],
		desc: '+2 to Illusion or Manipulation spells, +1 to resist Poison',
		properties: 'Fast, Adaptable, Trickster, Hardy'},
	  :goose => {
		environment: 'Anywhere near water', spells: [[:Combat,1],[:Detection,2]],
		spirits: [[:Choose,[[:Land1,1],[:Sky,1],[:Water,1]]]], desc: '+1 Land, 
    Sky or Water spirits, +2 to all Magical TNs away from home/city region, 
    28 days to reset'.delete("\n\t"),
	  ...
	  :jackal => {
		environment: 'Savannah', spells: [[:Combat,-1],[:Detection,2],[:Illusion,2]],
		spirits: [[:Prairie,2]],
	  ...
	  :lizard => {
		...
		spirits: [[:Choose,[[:Desert,2],[:Forest,2],[:Mountain,2]]]], desc: '+2 to 
    all TNs in tight quarters. When trapped with no clear view of the sky, 
    shaman must make Willpower(6) Test or fly into panic for 3 turns, -1 per 
    success. Shaman will do everything to escape.'.delete("\n\t"),
	  ...
	  :otter => {
		environment: 'On or near water', spells: [[:Illusion,2],[:Combat,-1]],
		spirits: [[:Choose,[[:River,2],[:Sea,2]]]],
    ...
    :prairiedog => {
    environment: 'Anywhere on land', spells: [[:Detection,2],[:Illusion,1],
    [:Combat,-2]], spirits: [[:Land,2]], req: [[:Charisma,4]],
    properties: 'Friendly, Playful, Loves Having Large Group of Friends'},
    :puma => {
    ...
    :shark => {
    environment: 'On or by the sea', spells: [[:Combat,2],[:Detection,2]],
    spirits: [[:Sea,2]], desc: 'When wounded or kill an opponent can go 
    Berserk, rage lasts 3 turns, every success on Willpower(4) test 
    substracts a turn from the rage, avoided with 3 success, will attack 
    closest with most powerful weapons, can choose to attack body of last 
    victim instead of living person',
    ...
    desc: '+2 To ALL Magical TNs when in open or away from shelter, +1 
    to ALL TNs if he doesnt have time to plan a situation',
    ...
    :turtle => {
    environment: 'On or near water', spells: [[:Combat,-2],[:Illusion,2]],
    spirits: [[:Water1,2]],
  ...
  :nature => {
    ...
    spells: [[:Illusion,2],[:Transformation,2],[:Detection,1],[:Combat,-1]],
    spirits: [[:Water,1]], desc: 'Willpower(6) test to engage in direct
    confrontation', properties: 'Secretive, Ever-Changing'},
    :mountain => {
  ...
  :idol => {
    ...
    [:Illusion,2]], spirits: [[:Storm,2]], desc: 'May go berserk in 
    same manner as Bear', properties: '
  ...
  :mythic => {
    ...
    desc: 'Willpower(8) test to back down or flee from confrontation,
    if wounded goes berserk like bear', properties: '
  ...
  :loa => {
    ...
    desc: 'Willpower(6) test to reveal good information', properties: '
    Graceful, Slow, Wise, Avoids Human Speech, Strict Morals, Enjoys 
